<template>
    <v-row justify="center">
        ...
        <v-card
            ...
            class="ma-3"
            max-width="1000px"
            v-if="this.active_sctn != null"
        >
            <v-card-title>
                <div v-if="this.active_sctn.title != null">
                    {{ this.active_sctn.title }}
                </div>
            </v-card-title>
            <v-data-table
                :headers="headers"
                ...
                <v-col>
                    <v-row cols="12" justify="end">
                        <v-card-actions class="ma-4">
                            <v-btn @click="navigate('assignments')"
                                >Cancel</v-btn
                            >
                            <v-btn
                                @click="saveInpsection()"
                                color="primary"
                                dark
                                >Save</v-btn
                            >
                            ...
                            <v-btn
                                ...
                                color="secondary"
                                v-if="
                                    master_sctn.status == 'Submitted' ||
                                        master_sctn.status == 'In Review'
                                "
                                >Mark Complete</v-btn
                            >
                            <v-btn
                    ...
                    <v-row cols="12" justify="center">
                        <v-card-actions md="12">
                            ...
                            <v-label class="pa-2"
                                >Section {{ this.sctn_index + 1 }} of
                                {{ this.master_sctn.sections.length }}</v-label
                            >
                            <v-btn
                                small
                                class="ma-2 white--text"
    ...
    methods: {
        ...
        prevSection() {
            if (this.sctn_index != 0) {
                this.active_sctn = this.master_sctn.sections[
                    this.sctn_index - 1
                ];
                this.sctn_index--;
            }
